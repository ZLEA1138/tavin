...
local function book_on_use(itemstack, user)
	...
	minetest.show_formspec(player_name, "sb_books:book", formspec_size .. formspec)
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname ~= "sb_books:book" then
		return
	end
	...
	local written = stack:get_name() == "sb_books:book_written"
	if stack:get_name() ~= "sb_books:book" and not written then
		-- No book in the wield slot, abort & inform the player
		minetest.chat_send_player(player_name, "The book you were writing to mysteriously disappeared.")
		return
	end
	...
	if fields.book_header ~= nil and data.owner == player_name then
		...
		minetest.show_formspec(player_name, "sb_books:book", formspec)
		return
	end
	...
	if fields.save and fields.title and fields.text then
		...
		if not written then
			...
			if count == 1 then
				stack:set_name("sb_books:book_written")
			else
				stack:set_count(count - 1)
				new_stack = ItemStack("sb_books:book_written")
			end
		end
...
-- Book item
minetest.register_craftitem("sb_books:book", {
	description = "Book",
...
functions.register_craft_metadata_copy("sb_books:book", "sb_books:book_written")

minetest.register_craft({
	type = "fuel",
	recipe = "sb_books:book",
	burntime = 10,
})

minetest.register_craft({
	type = "fuel",
	recipe = "sb_books:book_written",
	burntime = 10,
})