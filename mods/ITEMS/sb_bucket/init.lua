...
function sb_bucket.register_liquid(source, flowing, itemname, inventory_image, name,
		groups, force_renew)
	local itemname_raw = itemname
	itemname = itemname and itemname:match("^:(.+)") or itemname
	sb_bucket.liquids[source] = {
		source = source,
		flowing = flowing,
		itemname = itemname,
		force_renew = force_renew,
	}
	sb_bucket.liquids[flowing] = sb_bucket.liquids[source]
	...
	if itemname ~= nil then
		minetest.register_craftitem(itemname_raw, {
			...
			on_place = function(itemstack, user, pointed_thing)
				...
				return ItemStack("sb_bucket:bucket_empty")
			end
		})
	end
end

minetest.register_craftitem("sb_bucket:bucket_empty", {
	description = "Bucket",
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		local liquiddef = sb_bucket.liquids[node.name]
		...
		and node.name == liquiddef.source then
			...
			if item_count > 1 then
				...
				giving_back = "sb_bucket:bucket_empty "..tostring(item_count-1)
...
minetest.register_craft({
	type = "fuel",
	recipe = "sb_bucket:bucket_lava",
	burntime = 60,
	replacements = {{"sb_bucket:bucket_lava", "sb_bucket:bucket_empty"}},
})
