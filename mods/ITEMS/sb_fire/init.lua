...
local function flood_flame(pos, _, newnode)
	...
	if minetest.get_item_group(newnode.name, "igniter") == 0 then
		minetest.sound_play("extinguish_flame",
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		if pointed_thing.type == "node" then
			...
			elseif minetest.get_item_group(node_under, "flammable") >= 1
				...
				minetest.set_node(pointed_thing.above, {name = "sb_fire:basic_flame"})
			end
		end
...
minetest.register_craft({
	...
	recipe = {
		{"sb_core:flint", "sb_core:ferrum_ingot"}
	}
})

-- Override coalblock to enable permanent flame above
-- Coalblock is non-flammable to avoid unwanted basic_flame nodes
minetest.override_item("sb_minerals:coal_block", {
	after_destruct = function(pos)
		pos.y = pos.y + 1
		if minetest.get_node(pos).name == "sb_fire:permanent_flame" then
			minetest.remove_node(pos)
		end
	end,
	on_ignite = function(pos)
		...
		if minetest.get_node(flame_pos).name == "air" then
			minetest.set_node(flame_pos, {name = "sb_fire:permanent_flame"})
		end
	end
})
...
if flame_sound then
	...
	function sb_fire.update_player_sound(player)
		...
		local fpos, num = minetest.find_nodes_in_area(
			areamin,
			areamax,
			{"sb_fire:basic_flame", "sb_fire:permanent_flame"}
		)
		-- Total number of flames in radius
		local flames = (num["sb_fire:basic_flame"] or 0) +
			(num["sb_fire:permanent_flame"] or 0)
		-- Stop previous sound
		...
		if flames > 0 then
			...
			-- Play sound
			local handle = minetest.sound_play("fire", {
				pos = fposmid,
	...
	minetest.register_globalstep(function(dtime)
		...
		for n = 1, #players do
			sb_fire.update_player_sound(players[n])
		end
	end)
...
function sb_fire.update_sounds_around() end
...
if fire_enabled then
	...
	minetest.register_abm({
		...
		action = function(pos)
			...
			if p then
				minetest.set_node(p, {name = "sb_fire:basic_flame"})
			end
		end
	})
